...
class CountDeltaVolBatch(SignalTemplate):
    ...
    param_names = ['target_lastprice','target_volume','target_bid_vol','target_ask_vol']
    ...
    def __init__(self, subscribe_list, param_list, duplicate=1):
        ...
        #    self[str(param)] = param
        #a=1
        self.targeting_series_lastprice = self.param_dict['target_lastprice']
        self.targeting_series_volume=self.param_dict['target_volume']
        self.targeting_series_bid_vol=self.param_dict['target_bid_vol']
        self.targeting_series_ask_vol=self.param_dict['target_ask_vol']
        a=1
    def on_message(self, message):
        ...
        if message['tag'] == 'md':
            ...
            self.cached_volume.append(message['volume'])
            self.cached_ask1.append(message['ask_vol'])
            self.cached_bid1.append(message['bid_vol'])
            ...
            to_publish[str('num_hidden_trade')] = hidden_trade
...
class CountDeltaVolWindowBatch(SignalTemplate):
    ...
    def on_message(self, message):
        ...
        print(self.cached_num_ask)
        print(self.cached_num_bid)
...
def CountDeltaVolWindow_sub(instrument,param_list):
    my_signal_vol_window = CountDeltaVolWindowBatch(
        [
            'signal:count_delta_vol.{}'.format(instrument)
        ],
        param_list
    )
...
if __name__ == '__main__':
    ...
    p2 = mp.Process(target=CountDeltaVolWindow_sub, args=(inst,[3,6],))
